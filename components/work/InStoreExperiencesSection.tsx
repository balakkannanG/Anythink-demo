        ...
        >
          {experiences.map((experience, index) => (
            <motion.article
              ...
              className={`group px-4 py-5 sm:px-6 lg:px-5 ${index > 0 ? "border-dashed borderbgimageleft  sm:border-l" : ""}`}
            ...
            >
              ...
              <div className="mt-7 text-center flex flex-col items-center justify-center gap-2">
                ...
                <div className="flex items-start justify-center gap-2 text-mustard h-20">
                  <MapPin className="mt-0.5 shrink-0" size={25} strokeWidth={1.4} aria-hidden="true" />
                  <h3 className="max-w-40 font-chalk text-2xl uppercase leading-[0.9] sm:text-[1.65rem] ">{experience.title}</h3>
                </div>
                <p className="mt-5 font-mono text-xl uppercase text-white/80">
                  for <span className="text-mustard">{experience.client}</span>
                </p>
              </div>
            </motion.article>
          ))}
